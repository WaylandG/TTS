...
//自己添加
//界面上的控件
@property(nonatomic,strong) UIImageView *imageView;//方块
@property(nonatomic,strong) UIButton *button;//三个按钮

@end

@implementation SDCycleScrollView
//界面上的控件
-(UIImageView*)imageViewView:(UIView*)view {
    if (!_imageView) {
        _imageView = [[UIImageView alloc]initWithFrame:CGRectMake((ScreenBoundWidth-75)/2, (ScreenBoundHeight-400)/2, 75, 75)];
        _imageView.backgroundColor = [UIColor clearColor];
        _imageView.image = [UIImage imageNamed:@"bim_module.png"];
//        _imageView.contentMode = UIViewContentModeScaleAspectFill;
        [_imageView setContentMode:UIViewContentModeScaleToFill];
        _imageView.clipsToBounds = YES;
//        [view addSubview:_imageView];
        //[view insertSubview:_imageView belowSubview:view2];
    }
    
    return _imageView;
}
-(UIButton *)buttonView:(UIView*)view{
    if (!_button) {
        for (int i =0; i<2; i++) {//BIM赛事隐藏改成2
            //设置按钮类型，此处为圆角按钮
            _button = [UIButton buttonWithType:UIButtonTypeRoundedRect];
            //    设置和大小
            CGRect frame = CGRectMake(30.0f
                                      ,_imageView.frame.origin.y+_imageView.frame.size.height+30+i*45+i*30
                                      , ScreenBoundWidth -60
                                      , 45.0f);
            ...
            if (i == 0) {
                ...
                [_button setTitle:@"游客" forState:UIControlStateNormal];
                _button.backgroundColor = [UIColor clearColor];
                _button.tintColor = [UIColor whiteColor];
            }else if (i == 2){
                //暂时隐藏
//                [_button setTitle:@"BIM赛事" forState:UIControlStateNormal];
//                _button.backgroundColor = RGBColor(236, 236, 236);
//                _button.tintColor = LDMainRED_COLOR;
            }
...
//            [view addSubview:_button];
            //[view insertSubview:_button belowSubview:view2];
        }
    }
...
//        if (button.tag ==0) {
//            NSLog(@"登录");
//            [LCProgressHUD showSuccess:[NSString stringWithFormat:@"登录"]];
//        }else if (button.tag ==1){
//            [LCProgressHUD showSuccess:[NSString stringWithFormat:@"注册"]];
//            
//        }else if (button.tag ==2){
//            [LCProgressHUD showSuccess:[NSString stringWithFormat:@"BIM"]];
//            
//        }
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:didScrollToButton:)]) {
        [self.delegate cycleScrollView:self didScrollToButton:button];
    } else if (self.buttonDidScrollOperationBlock) {
        self.buttonDidScrollOperationBlock(button);
    }
...
- (void)layoutSubviews
{
    ...
    CGFloat y = self.mainView.sd_height - size.height - 15;//修改原点位置
...
//        cell.imageView.contentMode = UIViewContentModeScaleAspectFill;
    ...
    //添加自定义的控件
    if (_isState) {//如果用户设置为yes需要处理
        if (itemIndex+1 == self.imagePathsGroup.count) {
            //最后一个
            [self imageViewView:cell];
